...
4. **Implement RJSF Form:** Render the `<Form schema={schema} validator={validator} onSubmit={handleSubmit} formData={formData} onChange={...} />`. Note that RJSF's `onSubmit` prop expects a handler signature of `(data: ISubmitEvent<T>, event: React.FormEvent) => void` (rather than a standard HTML form `onSubmit` handler which receives `React.FormEvent` as the first argument), so `handleSubmit` will be updated accordingly.
...
2. **Resolve Array Item Schema:** Determine the array-typed schema. If the top-level schema is a direct array (i.e. `schema.type === 'array'`), resolve its item schema (e.g. `schema.items.properties`). If it is an object wrapper (e.g. `{ data: [...], total: 100 }`), traverse properties to identify the nested array property and resolve its item schema.
...
**Implementation Plan:**
1. **Install Dependencies:** Run `npm install -D jest jest-environment-jsdom @testing-library/react @testing-library/dom @testing-library/jest-dom`. (Note that since Next.js has built-in support for Jest using SWC, `ts-jest` is omitted to avoid compiler and configuration conflicts).
2. **Configure Jest:** Create `jest.config.ts` configured for Next.js (using `next/jest`).
...
3. **Update Data Fetching:** Modify `fetchData` to append these query parameters safely to the URL using browser-native APIs like `URL` or `URLSearchParams` to ensure parameters are properly encoded and handled regardless of whether the URL already contains existing query parameters.
